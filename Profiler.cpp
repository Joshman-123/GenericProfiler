...
namespace prf
{
    ...
    {
        static std::once_flag l_initFlag;
        std::call_once(l_initFlag, [&f_input]()
                       {
            if (f_input.m_logCallback)
            {
    ...
    void BlockProfiler::print(const Precision f_precision, const std::vector<std::string> &f_blockNames)
    {
        ...
        if (f_precision == Precision::Micro)
        {
            l_divisor = 1000.0;
            l_unit = "us";
        }
        else if (f_precision == Precision::Milli)
        {
            l_divisor = 1000000.0;
            l_unit = "ms";
        }
    ...
    ScopeProfile::ScopeProfile(const char *const f_blockName, const char *const f_scopeName) : m_profiler(BlockProfiler::getInstance(f_blockName)), // Cache reference to avoid map lookup on destruction
